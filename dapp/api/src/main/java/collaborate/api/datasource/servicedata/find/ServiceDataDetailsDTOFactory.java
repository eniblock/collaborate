...
import collaborate.api.datasource.servicedata.model.AccessStatus;
import collaborate.api.datasource.servicedata.model.ServiceDataDetailsDTO;
import collaborate.api.datasource.servicedata.model.TokenStatus;
...
@Service
@Slf4j
public class ServiceDataDetailsDTOFactory {
  ...
  public List<ServiceDataDetailsDTO> makeFromFA2(Collection<Integer> tokenIdList) {
    /**
     * FIXME:
    ...
    var tokenOwnersByTokenId = findServiceDataDAO.getOwnersByTokenIds(tokenIdList);

    var tokenOperatorsByTokenId = findServiceDataDAO.getOperatorsByTokenIdsAndOwners(tokenIdList,
    ...
    return tokenIdList.stream()
        .map(tokenId -> {
              ...
              var creationDatetime =
                  serviceDataTransactionService
                      .findTransactionDateByTokenId(
                          serviceDataContractAddress,
                          Long.valueOf(tokenId)
                      );
              return ServiceDataDetailsDTO.builder()
  ...
  public List<ServiceDataDetailsDTO> makeFromMultiSig(
