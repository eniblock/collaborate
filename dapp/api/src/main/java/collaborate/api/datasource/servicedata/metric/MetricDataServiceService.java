package collaborate.api.datasource.servicedata.metric;
...
public class MetricDataServiceService {
  ...
  public Optional<Page<Metric>> findAll(Integer tokenId, Pageable pageable, String query) {
    ...
    var servicedataDtoOpt = findServiceDataService.findServiceDataDetailsByTokenId(tokenId);
    if (servicedataDtoOpt.isEmpty()) {
      log.info("No servicedata found for tokenId={}", tokenId);
      return Optional.empty();
    } else {
      var servicedataDetailsDTO = servicedataDtoOpt.get();
      ensureConnectedUserAllowed(servicedataDetailsDTO);
      var pagedMetricsAndUri =
          buildMetricUrls(servicedataDetailsDTO)
      ...
      return Optional.of(
          new PageImpl<>(
              ...
              pageable,
              servicedataDetailsDTO.countScopes()
          )
      );
    }
  }

  void ensureConnectedUserAllowed(ServiceDataDetailsDTO servicedataDetailsDTO) {
    var currentUserWallet = connectedUserService.getWalletAddress();
    var isAllowed =
        currentUserWallet.equals(servicedataDetailsDTO.getAssetOwner().getAddress())
            || currentUserWallet.equals(servicedataDetailsDTO.getOperator().getAddress());
    if (!isAllowed) {
  ...
  Stream<GatewayResourceDTO> buildMetricUrls(
      ServiceDataDetailsDTO servicedataDetailsDTO) {
    return servicedataDetailsDTO
        .getAssetDataCatalog()
