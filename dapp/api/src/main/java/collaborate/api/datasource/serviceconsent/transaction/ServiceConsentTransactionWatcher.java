...
@Slf4j
@Component
@ConditionalOnExpression("!'${smartContractAddress.ServiceConsent}'.isEmpty()")
public class ServiceConsentTransactionWatcher {

  private final String serviceConsentContractAddress;
  private final MintServiceConsentTokenHandler mintTokenHandler;
  ...
  public void onApplicationEvent(ContextRefreshedEvent event) {
    if (StringUtils.isNotBlank(serviceConsentContractAddress)) {
  ...
  private TransactionWatcher buildWatcher() {
    return new TransactionWatcher(
        ...
        initServiceConsentEventManager(),
  ...
  private TransactionEventManager initServiceConsentEventManager() {
