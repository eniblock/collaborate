...
public class EmailAuthenticator implements
    Authenticator /*, CredentialValidator<EmailCredentialProvider>*/ {

  private static final Logger logger = Logger.getLogger(EmailAuthenticator.class);

  @Override
  public void authenticate(AuthenticationFlowContext context) {
    Integer codeLength = ConfigurationUtil.getConfigInteger(context.getAuthenticatorConfig(),
        Constants.CONF_MAIL_CODE_LENGTH);
    ...
    EmailTemplateProvider emailTemplateProvider = context.getSession()
  ...
  private void storeSMSCode(AuthenticationFlowContext context, String code) {
    ...
    context.getSession().userCredentialManager()
        .updateCredential(context.getRealm(), context.getUser(), credentials);
  }
  ...
  protected CODE_STATUS validateCode(AuthenticationFlowContext context) {
    ...
    List<CredentialModel> codeCreds = session.userCredentialManager()
        .getStoredCredentialsByType(context.getRealm(), context.getUser(),
            Constants.TYPE_MAIL_CODE);
    ...
    Long codeTTLMinute = ConfigurationUtil.getConfigLong(context.getAuthenticatorConfig(),
        Constants.CONF_MAIL_CODE_TTL);

    if (enteredCode == null || !enteredCode.equals(secretData)) {
      return CODE_STATUS.INVALID;
    ...
    switch (status) {
      ...
      case INVALID:
        if (context.getExecution().getRequirement()
            ...
            context.getExecution().getRequirement()
